...
static Anim_t *
LoadGIF_IO_Internal(SDL_IOStream *src, bool load_anim)
{
...
static Animation_t *LoadGIFAnimation_IO(SDL_IOStream *src)
{
    Anim_t *internal = LoadGIF_IO_Internal(src, true);
    if (internal) {
...
{
        Animation_t *anim;
	SDL_IOStream* src = RWFromIdf(instead_idf(), file);
	if (!src)
		return NULL;
