...
INSTEAD позволяет открывать игру по любому пути из меню "Выбор игры".
...
На самом деле, 'path' создаёт комнату с атрибутом 'disp', который
...
В данном примере, мы создали _отключённый_ переход, за счёт вызова
...
Здесь мы закрываем и открываем не переход, а комнату, в которую ведёт
...
Эта настройка действует на все диалоги, устанавливайте её в init() или
start() функции.
...
Ещё одна частая ситуация, вы хотите, чтобы фраза не пряталась после её
...
Таким образом можно записывать диалоги не древовидно, а линейно. Ещё
...
> фразами включёнными внутрь. Но так как при показе фраз движок
> остановится на выключенном/закрытом объекте-фразе и не войдёт
...
Системные объекты, это объекты, чьё имя начинается с символа
...
Пусть мы хотим сделать ссылку, при нажатии на которую мы перейдём в
...
Объекты, чьё имя начинается на символ '$' тоже считаются системными
...
объект $math который в своём act методе превращает текст в графическое
...
или вспомогательную переменную-счётчик. Для определения местоположения
...
или 'false'). Если вы вернёте true, то это будет признаком важного
...
Если вы вернёте false, то цепочка life методов прервётся на вас. Это
...
описанные действия. Вам остаётся только вернуть false.
...
Если в текущей сцене не определён атрибут 'pic', то берётся атрибут
'game.pic'. Если не определён и он, то картинка не отображается.

Поддерживаются все наиболее распространённые форматы изображений, но я
...
изображениям, разделённых символом ';'. Вторая и последующие
...
Интерпретатор проигрывает в цикле текущую музыку, которая задаётся
...
проигрывании определённых файлов (ошибки в связке библиотек SDL_mixer
и libmikmod).
...
проигрываний (циклов). Получить текущий счётчик можно с помощью
...
распространённых звуковых форматов также будет работать.
...
текущий проигрываемый трек. Выйдя из игры и загрузив её снова, игрок
...
остановки звука в определённом канале 'snd.stop(канал)'.

__Важно!__

> Если вы используете зацикленные звуки, вам придётся самим
...
Если вам недостаточно описанных здесь функций по работе со звуком,
...
(параграфам). В противном случае, результат не определён. Разбивайте
текст на абзацы символами '^' или 'pn()'.

INSTEAD при выводе удаляет лишние пробелы. Это значит, что неважно
сколько пробелов вы вставляете между словами, всё равно при выводе они
не будут учитываться для расчёта расстояния между словами. Иногда это
...
Ещё один пример. Если вы используете подчёркивание текста, то
промежутки между словами не будут подчёркнуты. При использовании
'fmt.nb' промежутки также будут подчёркнуты.
...
Необязательный строковой параметр _центр_ задаёт позицию в следующем
...
Необязательный строковой параметр _центр_ задаёт позицию внутри
...
сохраняется, но вы можете задать её для игры целиком, или
восстанавливать её вручную в функции 'start()'.

Если вас категорически не устраивает то, как INSTEAD формирует вывод
(последовательность абзацев текста), вы можете переопределить функцию
...
Тот факт, что я привёл здесь этот код, не означает, что я рекомендую
...
в виде подстановок $fmt и размещать их прямо в тексте.
...
Конструктор -- это функция, которая создаёт за вас объект и заполняет
...
```
Как видим, идея конструкторов проста. Вы просто создаёте функцию,
которая получает на вход таблицу с атрибутами {}, которую конструктор
может дозаполнить нужными атрибутами. Затем эта таблица передаётся
...
'std.call(объект, метод, параметры)', при этом эта функция вернёт
...
на ZX-"Спектрум". В игре "Особняк" своё управление, которое вводит
...
Иногда возникает желание вывести какой-нибудь статус. Например,
...
> Не используйте компиляцию игр с помощью 'luac', так как 'luac' создаёт
...
Существует ещё один специальный статус: std.nop(). Он может быть
...
оформление игры на лету и перерисовать кадр с учётом новых параметров
темы.
...
он закончился или прошло более 2 минут и игрок перешёл из комнаты в
...
создаёте их в глобальном пространстве lua файла. Однако, существуют
...
> ещё не загружено полностью! В качестве параметров поддерживаются
...
игре. Например, если речь идёт о сценах, где важный элемент составляет
...
std.is_obj() удобная для определения типа переменной или аргумента
функции.
...
наследуются от неё и могут частично или полностью заменять её
...
Цвет задаётся в форме #rgb, где r g и b компоненты цвета в
...
- scr.gfx.mode = режим расположения (строка fixed, embedded или
  float). Задаёт режим изображения. embedded -- картинка является
...
- win.gfx.up, win.gfx.down = пути к файлам-изображениям скроллеров
...
- inv.fnt.height = междустрочный интервал как число с плавающей
  запятой (1.0 по умолчанию)
- inv.gfx.up, inv.gfx.down = пути к файлам-изображениям скроллеров
...
события нажатия клавиши и false -- отжатия. В key передаётся
...
клавиатуры, а опрашивать её (как правило, в таймере). Для этого вы
...
_Внимание!_ В INSTEAD по умолчанию включён фильтр кликов мыши, который
...
> -- создайте свою тему и включите её в игру!
...
собственный файл 'theme.ini', функция вернёт точку. Это удобно, для
определения того, включён ли механизм собственных тем игр:
...
- :alpha(alpha) - создаёт новый спрайт с заданной прозрачностью alpha
...
- :scale(xs, ys, [smooth]) -- масштабирует спрайт, для отражений
  используйте масштаб -1.0 (медленно! не для реального времени). Создаёт
  новый спрайт.
- :rotate(angle, [smooth]) -- поворот спрайта на заданный угол в
  градусах (медленно! не для реального времени). Создаёт новый спрайт.
...
- :compose(fx, fy, fw, fh, dst\_spr, x, y) -- Рисование - с
  учётом прозрачности обоих спрайтов). Существует укороченный вариант
  (как :draw).
...
Кроме того, существует возможность работы с шрифтами. Шрифт создаётся
...
Она возвращает размер шрифта с учётом масштабирование, которое
...
когда выбран предмет из инвентаря (курсор изменил вид на шестерёнки)
...
Если режим удалось включить -- функция вернёт true. sprite.direct()
...
При этом в меню интерпретатора INSTEAD указатель мыши всё ещё будет
виден.
...
> компоненты пикселя: красный, зелёный, синий, и прозрачность. Все
...
- :pixel(x, y, r, g, b, a) -- нарисовать пиксель (с учётом
...
snd имеет ещё некоторые функции по работе со звуком.
...
```
Звук задаётся в нормированном формате: [-1 .. 1]
...
- snd.playing([channel]) – узнать проигрывается ли звук на любом
  ...
  функция вернёт хандл (дескриптер) проигрываемого в данный момент звука или
...
Ещё одна интересная возможность -- генерирование звука на лету (пока
...
сохранённая информация не зависит от состояния игры. Такой механизм
можно использовать, например, для реализации системы достижений или
счётчика количества прохождений игры.
...
предварительно сохранённые состояния игры. В качестве примера, можно
...
вернётся к предварительно сохранённому состоянию игры.
...
- :push(фраза) - перейти к фразе с запоминанием её в стеке;
...
Вот и всё, здесь документация заканчивается. Но, возможно, начинается
...
текстовые приключения всё ещё существуют. Правда, их влияние на
культуру по-прежнему минимально. И хороших приключений -- по-прежнему
очень мало.

На мой взгляд, у тексто-графических игр огромный потенциал. Они не
такие интерактивные, они не отбирают вашу жизнь взамен на вечно
неудовлетворённое желание, не вынуждают вас сутками просиживать за
...
движок. Я знаю, что это время потрачено не зря. Так что я нашёл в себе
силы, и сделал движок ещё лучше, выпустив STEAD3. Я надеюсь, он
понравится и вам.
...
Спасибо и удачи. Пётр Косых, март 2017.
